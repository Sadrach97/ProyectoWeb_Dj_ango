...
from .serializers import ClienteSerializers, ClienteSerializers2,ProductoSerializer,ProductoSerializers2
...
def addProducto(request):
    data1 = JSONParser().parse(request)
    serializer = ProductoSerializers2(data = data1)
    if serializer.is_valid():
        serializer.save()
        return Response(serializer.data, status = status.HTTP_201_CREATED)
    else:
...
@api_view(['GET','PUT','DELETE'])
@permission_classes((IsAuthenticated,))
def controlarProducto(request,id):
    try:
    ...
    if request.method == 'GET':
        serializer = ProductoSerializers2(m)
        return Response(serializer.data)

    elif request.method == 'PUT':
        data2 = JSONParser().parse(request)
        serializer = ProductoSerializers2(m,data = data2)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        else:
